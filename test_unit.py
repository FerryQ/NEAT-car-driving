...
class TestSensor:
    ...
    def test_sensor_calculate_line_forward(self, mock_surface):
        ...
        expected_end_x = expected_start_x + sensor.length * math.cos(math.radians(angle))
        expected_end_y = expected_start_y - sensor.length * math.sin(math.radians(angle)) 
...
def normal_car(mock_surface, mock_game_map):
    return NormalCar(max_vel=5, rotation_vel=4, start_pos=(100, 100),
                     car_image=mock_surface, game_map=mock_game_map, best_car=mock_surface)

@pytest.fixture
def drift_car(mock_surface, mock_game_map):
    return DriftCar(max_vel=6, rotation_vel=4, start_pos=(100, 100),
                    car_image=mock_surface, game_map=mock_game_map, best_car=mock_surface)
...
class TestNormalCarMovement:
    def test_normal_car_accelerate(self, normal_car):
        ...
        assert math.isclose(normal_car.position.y, initial_pos.y) # No vertical movement if angle is 0
...
class TestDriftCarMovement:
    ...
    def test_drift_car_friction_and_drift(self, drift_car):
        ...
        assert not math.isclose(drift_car.position.x, initial_position.x) or \
               not math.isclose(drift_car.position.y, initial_position.y)
